 ...
 * {@code aware.ingest.simulator.enabled=false}.
 *
    ...
    void burstIsUnavailableWithoutTheSimulator() throws Exception {
        mockMvc.perform(post(BASE + "/simulate/burst").param("stormStations", "3"))
                ...
                .andExpect(jsonPath("$.detail").value(
                        "Burst requires aware.ingest.simulator.enabled=true; "
                                + "the service is in REST-only mode"))
                .andExpect(jsonPath("$.type").value("urn:aware:problem:simulator-disabled"));
    }
