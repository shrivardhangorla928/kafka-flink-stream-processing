...
/**
 * Every operator-tunable number the ingest service uses, bound from {@code stream.ingest.*}.
 *
...
public class IngestProperties {
    ...
    @NotBlank
    private String source = "STREAM_SIMULATOR";
    ...
    public static class SimulatorProperties {
        ...
        @AssertTrue(message = "stream.ingest.simulator.storm-max-cycles must be >= storm-min-cycles")
        public boolean isCycleRangeOrdered() {
            return stormMaxCycles >= stormMinCycles;
        }

        @AssertTrue(message = "every stream.ingest.simulator storm band must have max >= min")
