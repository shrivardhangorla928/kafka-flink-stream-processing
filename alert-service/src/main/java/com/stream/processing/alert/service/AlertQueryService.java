...
import com.stream.processing.alert.config.AlertServiceProperties;
...
import com.stream.processing.alert.repository.AlertSpecifications;
import com.stream.processing.alert.repository.StationWindowAggregateRepository;
import com.stream.processing.alert.web.AlertNotFoundException;
...
public class AlertQueryService {
     ...
     *
     * @param limit caller-supplied bound, clamped to {@code stream.alert.api.max-timeline-size}
     */
    @Transactional(readOnly = true)
    public List<StationWindowDto> timeline(String stationId, SensorType sensorType, int limit) {
        ...
        List<com.stream.processing.alert.domain.StationWindowAggregateEntity> rows = sensorType == null
