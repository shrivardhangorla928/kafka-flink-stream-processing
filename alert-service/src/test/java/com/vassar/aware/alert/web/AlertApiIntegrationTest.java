...
class AlertApiIntegrationTest extends AbstractIntegrationTest {
    ...
    void theRequestedPageSizeIsCappedServerSide() throws Exception {
        mockMvc.perform(get(BASE).param("size", "100000"))
                .andExpect(status().isOk())
                // aware.alert.api.max-page-size, not the 100000 the caller asked for.
    ...
    void prometheusExposesThePipelineLatencyAndAlertCounters() throws Exception {
        ...
        assertThat(scrape).contains("aware_alerts_pipeline_latency_seconds");
        assertThat(scrape).contains("aware_alerts_duplicate_total");
        assertThat(scrape).contains("aware_alerts_persist_failures_total");
